    ...
    int totalVerts)
{
    ...
    parts.reserve(N);
    for (int p = 0; p < N; ++p) {
        parts.push_back({vertBounds[p], vertBounds[p + 1], faceStart[p], faceStart[p + 1]});
    }
...
inline bool loadPenguinObj(const std::string& path, std::vector<Mesh>& outMeshes)
{
    ...
    while (std::fgets(line, sizeof(line), f)) {
        if (line[0] == 'v' && line[1] == ' ') {
            ...
            int a, b, c, d;
            int na, nb, nc, nd;
            if (std::sscanf(line + 2, "%d//%d %d//%d %d//%d %d//%d", &a, &na, &b, &nb, &c, &nc, &d, &nd) == 8) {
                allFaces.push_back({a, b, c});
                allFaces.push_back({a, c, d});
            } else if (std::sscanf(line + 2, "%d//%d %d//%d %d//%d", &a, &na, &b, &nb, &c, &nc) == 6) {
                allFaces.push_back({a, b, c});
    ...
    if (parts.size() != specs.size()) {
        std::fprintf(stderr, "obj_loader: expected %zu parts, detected %zu. " "Check that the OBJ has contiguous vertex blocks per body part.\n", specs.size(), parts.size());
        return false;
    }
    ...
    for (size_t p = 0; p < parts.size(); ++p) {
        ...
        std::vector<Vec3> block(allVerts.begin() + pr.vertBegin, allVerts.begin() + pr.vertEnd);
        ...
        mesh.pivot = { mn.x + (mx.x - mn.x) * r.fx, mn.y + (mx.y - mn.y) * r.fy, mn.z + (mx.z - mn.z) * r.fz };
        ...
        std::printf("  [%2zu] %-12s  %zu verts, %zu faces\n", p, mesh.name.c_str(), mesh.vertices.size(), mesh.faces.size());
